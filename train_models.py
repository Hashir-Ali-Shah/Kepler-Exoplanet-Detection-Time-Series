# =============================================================================
# Exoplanet Detection - 4-Model Training Script
# =============================================================================
# Models:
#   Model 1: LSTM  + SMOTE + Focal Loss
#   Model 2: GRU   + SMOTE + Focal Loss
#   Model 3: LSTM  + Windowed Oversampling + Focal Loss
#   Model 4: GRU   + Windowed Oversampling + Focal Loss
#
# Features:
#   - Gaussian smoothing (sigma=2)
#   - Row-wise normalization
#   - Epoch-level checkpointing (safe resume after internet disconnect)
#   - Per-class Precision, Recall, F1 at threshold=0.5
#   - PR Curve and ROC Curve per model
# =============================================================================
...
# 3-D for LSTM/GRU: (samples, timesteps, 1)
...
# =============================================================================
# STEP 2 — BUILD OVERSAMPLED TRAINING SETS
# =============================================================================
...
# --- Windowed oversampling branch ---
def windowed_oversample(X_scaled, y_arr, window_size=200, step=50):
    """
    Oversample the minority class by extracting overlapping windows from
    each minority sample and treating each window as an augmented full-length
    sample via linear interpolation back to original length.
    """
    ...
    for idx in minority_idx:
        ...
        while start + window_size <= T:
            ...
            # Resize window back to T via linear interpolation
...
def compile_model(model):
    model.compile(
        # clipnorm=1.0 prevents gradient explosions (sudden accuracy collapse)
...
# =============================================================================
# STEP 4 — CHECKPOINT-AWARE TRAINING
# =============================================================================

def get_initial_epoch(ckpt_dir):
    """Return the last completed epoch number (0 = not started)."""
...
def load_or_build(model_name, builder, ckpt_dir):
    """Load weights from checkpoint if available, else build fresh."""
...
def train_model(model_name, model, X_tr, y_tr, ckpt_dir):
    """Train with per-epoch checkpointing; resumes automatically."""
    ...
    # --- Callback 1: save latest weights every epoch (for resume) ---
    class EpochCheckpoint(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            self.model.save_weights(weights_path)
            save_meta(ckpt_dir, epoch + 1)  # +1 because epoch is 0-indexed

    # --- Callback 2: print Class-1 (exoplanet) Precision/Recall/F1 each epoch ---
    ...
    # --- Callback 2: also save best weights to a separate file ---
    # EarlyStopping's restore_best_weights only lives in memory;
    # saving to disk lets us reload the best weights after a disconnect.
    best_val_auc = [0.0]
    ...
    # --- Callback 3: reduce LR when val_auc plateaus ---
    ...
    # --- Callback 4: stop early and restore best weights ---
    early_stop = tf.keras.callbacks.EarlyStopping(
        monitor="val_auc",
        patience=15,
        restore_best_weights=True,  # in-memory restore
        mode="max",
        verbose=1,
    )
    ...
    # After fit, load the best weights from disk to be safe
    # (in case EarlyStopping's in-memory restore was lost on disconnect)
...
MODELS = [
    {
        ...
        "X_tr":    X_train_smote,
        "y_tr":    y_train_smote,
        "ckpt":    os.path.join(CHECKPOINT_DIR, "lstm_smote"),
    },
    {
        "name":    "GRU_SMOTE",
        "builder": build_gru,
        "X_tr":    X_train_smote,
        "y_tr":    y_train_smote,
        "ckpt":    os.path.join(CHECKPOINT_DIR, "gru_smote"),
    },
    {
        "name":    "LSTM_Windowed",
        "builder": build_lstm,
        "X_tr":    X_train_win,
        "y_tr":    y_train_win,
        "ckpt":    os.path.join(CHECKPOINT_DIR, "lstm_windowed"),
    },
    {
        "name":    "GRU_Windowed",
        "builder": build_gru,
        "X_tr":    X_train_win,
        "y_tr":    y_train_win,
        "ckpt":    os.path.join(CHECKPOINT_DIR, "gru_windowed"),
    },
]
