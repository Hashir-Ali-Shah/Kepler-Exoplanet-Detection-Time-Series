...
COMPARE_DIR = "/kaggle/working/comparison"
GAUSSIAN_SIGMA = 2
RANDOM_STATE = 42
...
ax.axhline(baseline, color="gray", ls="--", alpha=0.5, label=f"Baseline (no-skill = {baseline:.3f})")
...
for ax, key, title in zip(axes, ["p1", "r1", "f1"], ["Precision (Class 1)", "Recall (Class 1)", "F1 Score (Class 1)"]):
    for name in models_loaded:
...
fig.suptitle("Class-1 Metrics vs Threshold — All Models", fontsize=14, fontweight="bold", y=1.02)
fig.tight_layout()
fig.savefig(os.path.join(COMPARE_DIR, "03_metrics_vs_threshold.png"), dpi=150, bbox_inches="tight")
plt.close(fig)
...
for ax, t in zip(axes, key_thresholds):
    for i, name in enumerate(models_loaded):
        ...
        for bar, v in zip(bars, vals):
            if v > 0.005:
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.01, f"{v:.2f}", ha="center", va="bottom", fontsize=7)
...
fig.legend(handles, labels, loc="upper center", ncol=len(models_loaded), fontsize=10, bbox_to_anchor=(0.5, 1.05))
fig.suptitle("Class-1 Metrics at Key Thresholds", fontsize=14, fontweight="bold", y=1.10)
fig.tight_layout()
fig.savefig(os.path.join(COMPARE_DIR, "04_bar_key_thresholds.png"), dpi=150, bbox_inches="tight")
plt.close(fig)

for plot_num, (metric_key, metric_label) in enumerate([("r1", "Recall"), ("p1", "Precision"), ("f1", "F1")], start=5):
    hm_data = {name: [metrics[name][t][metric_key] for t in THRESHOLDS] for name in models_loaded}
    ...
    ax.set_title(f"Class-1 {metric_label} — Models × Thresholds", fontsize=14, fontweight="bold", pad=20)
    ax.set_xlabel("Model", fontsize=12, labelpad=10)
    ax.set_ylabel("Threshold", fontsize=12, labelpad=15)
