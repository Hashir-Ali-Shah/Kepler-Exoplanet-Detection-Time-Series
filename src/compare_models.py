...
RESULTS_DIR = r"d:\ML\Tasks\LSTM\models"
COMPARE_DIR = r"d:\ML\Tasks\LSTM\results\images"
GAUSSIAN_SIGMA = 2
RANDOM_STATE = 42
...
for ax, t in zip(axes, key_thresholds):
    for i, name in enumerate(models_loaded):
        ...
        for bar, v in zip(bars, vals):
            if v > 0:
                ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.005, f"{v:.2f}", ha="center", va="bottom", fontsize=7)
...
fig.legend(handles, labels, loc="upper center", ncol=len(models_loaded), fontsize=10, bbox_to_anchor=(0.5, 1.04))
...
for plot_num, (metric_key, metric_label) in enumerate([("r1", "Recall"), ("p1", "Precision"), ("f1", "F1")], start=5):
    data_hm = {name: [metrics[name][t][metric_key] for t in THRESHOLDS] for name in models_loaded}
    ...
    ax.set_title(f"Class-1 {metric_label} — All Models × All Thresholds", fontsize=14, fontweight="bold", pad=20)
